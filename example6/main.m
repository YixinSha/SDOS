...
% Parameters
a = 0.01;                             % lattice constant
c = 343;                              % speed of sound
nf = 21;                              % number of frequency
nk = 21;                              % number of wavevector
freq = c/a*linspace(0.45, 0.70, nf);  % normalized frequency
...
noIdx_cellNeg2=find(no2xyz(2,:)>=1.5*a*sqrt(3)+posTol);
noIdx_cellNeg1=find(no2xyz(2,:)<=1.5*a*sqrt(3)+posTol & no2xyz(2,:)>=0.5*a*sqrt(3)+posTol);
noIdx_cell0=find(no2xyz(2,:)<=0.5*a*sqrt(3)+posTol & no2xyz(2,:)>=-0.5*a*sqrt(3)-posTol);
noIdx_cellPos1=find(no2xyz(2,:)<=-0.5*a*sqrt(3)-posTol & no2xyz(2,:)>=-1.5*a*sqrt(3)-posTol);
noIdx_cellPos2=find(no2xyz(2,:)<=-1.5*a*sqrt(3)-posTol);

% Match node indexes in negative direction
[~,noIdx_tmpCellNeg]=min(pdist2(no2xyz(:,noIdx_cellNeg2)',no2xyz(:,noIdx_cellNeg1)'+[0,a*sqrt(3)])');
noIdx_cellNeg1=noIdx_cellNeg1(:,noIdx_tmpCellNeg);

% Match node indexes in positive direction
[~,noIdx_tmpCellPos]=min(pdist2(no2xyz(:,noIdx_cellPos1)',no2xyz(:,noIdx_cellPos2)'+[0,a*sqrt(3)])');
...
noIdx_leftEdge=find(abs(no2xyz(1,:)-(-1.5*a))<posTol);
noIdx_rightEdge=find(abs(no2xyz(1,:)-(1.5*a))<posTol);
...
dof_cellNeg2LeftEdge=length(find( abs(no2xyz(1,:)-(-1.5*a))<posTol & no2xyz(2,:)>=1.5*a*sqrt(3)+posTol));
dof_cellNeg1LeftEdge=dof_cellNeg2LeftEdge;
dof_cell0LeftEdge=length(find( abs(no2xyz(1,:)-(-1.5*a))<posTol & no2xyz(2,:)<=0.5*a*sqrt(3)+posTol & no2xyz(2,:)>=-0.5*a*sqrt(3)-posTol));
dof_cellPos2LeftEdge=length(find( abs(no2xyz(1,:)-(-1.5*a))<posTol & no2xyz(2,:)<=-1.5*a*sqrt(3)-posTol));
